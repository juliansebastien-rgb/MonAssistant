  ...
  btn.addEventListener('click', function(){
    ...
    public static function maybe_render_admin_assistant_page() {
        ...
        echo '<!doctype html><html><head><meta charset="utf-8"/><meta name="viewport" content="width=device-width,initial-scale=1"/>'
            ...
            . '.card{background:#fff;border:1px solid rgba(18,61,100,.14);border-radius:16px;overflow:hidden;box-shadow:0 8px 28px rgba(18,61,100,.12)}'
            . '.azsa-head{display:flex;align-items:center;justify-content:space-between;gap:10px;padding:12px 14px;background:linear-gradient(135deg,#123d64 0%,#1c6ea4 100%);color:#fff}'
            . '.azsa-head h4{margin:0;font-size:15px;font-weight:700}.azsa-head small{display:block;opacity:.9;font-size:11px}.azsa-head-actions{display:flex;gap:6px;align-items:center}'
            . '#azsa-admin-mode,#azsa-admin-mic{border:1px solid rgba(255,255,255,.28);border-radius:10px;background:rgba(255,255,255,.16);color:#fff;height:30px;font-size:11px;padding:0 9px;cursor:pointer}'
            . '#azsa-admin-mic{width:34px;padding:0}.azsa-hidden{display:none!important}'
            . '.thread{height:62vh;overflow:auto;padding:12px;display:flex;flex-direction:column;gap:10px;background:linear-gradient(180deg,#f7fbff 0%,#ffffff 45%)}'
            . '.row{display:flex}.u{justify-content:flex-end}.b{justify-content:flex-start}'
            . '.bubble{max-width:86%;border-radius:14px;padding:9px 11px;font-size:14px;line-height:1.45;white-space:pre-wrap}'
            . '.u .bubble{background:#123d64;color:#fff;border-bottom-right-radius:6px}.b .bubble{background:#edf6ff;color:#123d64;border:1px solid rgba(18,61,100,.14);border-bottom-left-radius:6px}'
            . '.chips{display:flex;flex-wrap:wrap;gap:7px;padding:0 12px 8px}.chip{border:1px solid rgba(18,61,100,.18);background:#f4f9ff;color:#123d64;border-radius:999px;font-size:11px;padding:6px 10px;cursor:pointer}'
            . '.status{padding:0 12px 8px;font-size:11px;color:rgba(18,61,100,.75)}.status.thinking{color:#1c6ea4;font-weight:700}'
            . '.form{display:flex;gap:8px;align-items:center;padding:10px;border-top:1px solid rgba(18,61,100,.12);background:#fff}'
            . '.form input{flex:1;min-width:0;border:1px solid rgba(18,61,100,.22);border-radius:12px;padding:9px 10px;font-size:14px;color:#123d64}'
            . '.form button{border:1px solid rgba(18,61,100,.24);border-radius:12px;background:#123d64;color:#fff;padding:9px 12px;font-size:12px;font-weight:700;cursor:pointer}'
            . '</style></head><body>'
            . '<div class="wrap"><div class="card"><div class="azsa-head"><div><h4>Assistant CRM IA</h4><small>Connecté à ' . esc_html((string) $cfg['site_name']) . '</small></div><div class="azsa-head-actions"><button id="azsa-admin-mode" type="button">Mode: Écrit</button><button id="azsa-admin-mic" type="button" class="azsa-hidden" aria-label="Activer micro">🎙</button></div></div><div class="thread" id="azsa-admin-thread"></div><div class="chips" id="azsa-admin-chips"></div><div class="status" id="azsa-admin-status">Prêt</div><form class="form" id="azsa-admin-form"><input id="azsa-admin-input" type="text" placeholder="Ex: donne moi la fiche du contact Sébastien"/><button type="submit">Envoyer</button></form></div></div>'
